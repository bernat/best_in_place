...
BestInPlaceEditor.prototype = {
  ...
  activate : function() {
    ...
    if (this.isNil) {
      to_display = "";
			...
			try {
				if ( this.element.data('html-attrs')['class'].indexOf("format_amount") >= 0 ){
  ...
  loadErrorCallback : function(request, error) {
    if(this.oldValue != '') this.element.html(this.oldValue);
		else this.element.html(this.nil);

		resetValidationErrors();
		
    // Display all error messages from server side validation
    $.each(jQuery.parseJSON(request.responseText), function(index, value) {
			$('#dialog-validation-errors .errors ul').append("<li>" + value.toString() + "</li>");
    });
...
BestInPlaceEditor.forms = {
  "input" : {
    activateForm : function() {
      ...
      this.userClicked = false;
			if( this.element.find("input[type='text']").val() == this.nil ) this.element.find("input[type='text']").val('');
  ...
  "select" : {
    activateForm : function() {
      var output = "<form action='javascript:void(0)' style='display:inline;'><select class='searchable'>";
      var selected = "";
