...
BestInPlaceEditor.prototype = {
  ...
  /**
   * If any elements have a data-type of img convert them to images.
   */
  processImages : function()
  {
    if( this.element.attr('data-type') == 'img')
    {
      var imgUrl = this.element.html();
      var url = this.element.attr('data-original-content');
      var formName = this.objectName + "[" + this.attributeName + "]";
      var responseKey = this.element.attr('data-custom-response-key');
      var imageSpan = '<span class="bip_image" data-url="'+ this.url +'" ';
      imageSpan += 'data-original-content="'+imgUrl+'" ';
      imageSpan += 'data-form-name="'+formName+'" ';
      imageSpan += 'data-custom-response-key="'+ responseKey+'"></span>';
      $(imageSpan).insertAfter(this.element);
      this.element.remove();
    }
  },
  
  initInplaceImageOptions : function() 
  {
    $(".bip_image").inplaceImageChanger({data: {_method: "put"}});
  },
  ...
  loadSuccessCallback : function(data) {
    ...
    var customResponseKey = this.element.attr('data-custom-response-key');
    if (response != null && customResponseKey != null) {
      this.element.html(response[customResponseKey]);
    }
