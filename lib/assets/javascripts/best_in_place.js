...
BestInPlaceEditor.prototype = {
    ...
    initOptions: function () {
        ...
        var customConfirm = self.element.data("bipCustomConfirm");
        if(customConfirm)
          self.customConfirm = eval(customConfirm) || self.customConfirm;
        self.optionalData = null
    ...
    setOptionalData: function(data){
      'use strict'

      self.optionalData = ''
      $.each(data, function(key, value){
        if(typeof value ===  'string')
          self.optionalData += '&' + key + '=' + value;
        else{
          $.each(value, function(){
            self.optionalData += '&' + key + '[]=' + this;
          })
        }
      });
    },
    ...
    loadSuccessCallback: function (data, status, xhr) {
        ...
        this.element.trigger(jQuery.Event("best_in_place:success"), [jQuery.parseJSON(data), status, xhr]);
...
BestInPlaceEditor.forms = {
    "input": {
        ...
        inputBlurHandler: function (event) {
            ...
            if (event.data.editor.okButton) {
                event.data.editor.blurTimer = setTimeout(function () {
            ...
            } else if (event.data.editor.cancelButton) {
                  event.data.editor.blurTimer = setTimeout(function () {
                      if (!event.data.editor.userClicked) {
                  ...
                  var customConfirm = event.data.editor.customConfirm;
                  if(customConfirm)
                    event.data.editor.customConfirm()
                  else
                    event.data.editor.update();
              }
        },
    ...
    "select": {
        activateForm: function () {
            ...
            if(this.customConfirm){
              var self = this;
              this.element.on('best_in_place:startUpdate', function(e){
                e.stopImmediatePropagation();
                self.update();
              });
              this.element.on('best_in_place:startAbort', function(e){
                e.stopImmediatePropagation();
                self.abort();
              });
            }
    ...
    "textarea": {
        ...
        blurHandler: function (event) {
            'use strict';
            $(this).trigger(jQuery.Event("best_in_place:blur"));
            if (event.data.editor.okButton) {
                event.data.editor.blurTimer = setTimeout(function () {
                    if (!event.data.editor.userClicked) {
